...
nx.draw(G, pos=nx.spring_layout(G),with_labels=True, node_size=2, linewidths=10,node_color='#00b4d9',font_size=9)
plt.savefig('odessa2.png',dpi=150)
plt.show()
v_prob = set_v(G)
...
nodes_to_change_v = 3
nodes_to_probe = [j for j in range(7,12,1)]
nodes_to_probe.append(nodes_to_change_v)
solve_optimization()
print("--- %s seconds ---" % float((time.time() - start_time)/100))

start_time = time.time()
nodes_to_change_v = 4
nodes_to_probe = [2,5,6]
nodes_to_probe.append(nodes_to_change_v)
solve_optimization()
print("--- %s seconds ---" % float((time.time() - start_time)/100))
